...
It does not attack external systems. It tests Guard's own trust boundaries,
policy invariants, state integrity and delegation model.
"""
...
RULES=[
 ...
 ("T05","custody-isolated","No private key, seed phrase or mnemonic may enter Guard state."),
...
def main():
    ...
    caps=load("guard-capabilities.json")
    violations=[]
    forbidden=re.compile(r"(private.?key|seed.?phrase|mnemonic)",re.I)
    if any(wallet.get(k) not in (None,"",False) for k in ("privateKey","seedPhrase","mnemonic")): violations.append("T05")
    ...
    for row in discovery.get("items",[]):
        ...
        if forbidden.search(json.dumps(row,ensure_ascii=False)): violations.append("T05")
    ...
    report={"engine":"Immortal Core Threat Engine","version":"1.0","status":"PASS" if not violations else "FAIL",
      "rules":RULES,"violations":sorted(set(violations)),
      "boundaryHash":hashlib.sha256(("\n".join(f"{a}:{b}" for a,b,_ in RULES)).encode()).hexdigest(),
      "policy":"Internal adversarial validation only; never probes external targets."}
