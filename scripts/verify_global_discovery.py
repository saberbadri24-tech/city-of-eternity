...
UA = "ANIL-X-Immortal-Guard/10.0 Verification"
...
def main():
    ...
    discovery["verification"] = {
        ...
        "officialSourceMatches": sum(
            x.get("verification") == "resolved-official-source" for x in verified
        ),
        ...
        "rule": (
            "Publisher/news pages are discovery evidence only. A claim page is treated as "
            "trusted only when its resolved domain is in the trusted source registry."
        ),
    }
    ...
    learning["verificationEngine"] = (
        "resolve -> canonical -> trusted-domain check -> claim/eligibility/reward signal extraction"
    )
    learning["safetyGate"] = (
        "No discovery item can auto-claim. Owner approval remains mandatory for opportunity-specific actions."
    )
