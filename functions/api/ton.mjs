...
function validAddress(a){return /^[EUk]Q[A-Za-z0-9_-]{46}$/.test(String(a||''));}
...
export default async function tonApi(req){
  ...
  try{
    ...
    return json({ok:true,address,transactions:txs.map(t=>{
      const incoming=Number(t?.in_msg?.value||0);
      const outgoing=Array.isArray(t?.out_msgs)?t.out_msgs.reduce((s,m)=>s+Number(m?.value||0),0):0;
      return {hash:t?.hash||'',utime:t?.utime||0,amount:incoming-outgoing};
    })});
